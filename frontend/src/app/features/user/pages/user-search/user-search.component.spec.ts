...
describe('UserSearchComponent', () => {
  ...
  beforeEach(async () => {
    userServiceMock.getUserStatus.mockReturnValue(of([]));
    userServiceMock.getAllUsers.mockReturnValue(of({ content: [], totalElements: 0 }));
  ...
  describe('Initialization', () => {
    ...
    it('should handle error when loading userStatus from service failed', () => {
      (userServiceMock.getUserStatus as any).mockReturnValue(throwError(() => ({ status: 500 })));
      component.getUserStatus();
      expect(toastrMock.error).toHaveBeenCalledWith('Ocorreu um erro ao carregar os status');
    });
  ...
  describe('Form validations and UI', () => {
    ...
    it('should render table with columns', () => {
      ...
      const headers = Array.from(compiled.querySelectorAll('th')).map((th) =>
        th.textContent?.trim(),
      );
    ...
    it('should render table with data', async () => {
      ...
      component.usersTable = new MatTableDataSource(mockUsers);
    ...
    it('should apply red background class when user status is INACTIVE', async () => {
      ...
      expect(rows[1].nativeElement.classList.contains('bg-red-100!')).toBe(true);
      expect(rows[1].nativeElement.classList.contains('text-red-600!')).toBe(true);
    });

    it('should render status column using the UserStatusPipe', async () => {
      component.displayedColumns = ['id', 'name', 'login', 'email', 'status', 'editar'];
      component.usersTable = new MatTableDataSource(mockUsers);
      ...
      expect(rows[1].nativeElement.textContent).toContain('Inativo');
    });
  ...
  describe('Actions', () => {
    it('should load users from service', () => {
      ...
      (userServiceMock.getAllUsers as any).mockReturnValue(of(mockResponse));
      component.getAllUsers();
      expect(component.usersTable.data).toEqual(mockResponse.content);
      expect(component.totalElements).toBe(1);
    });

    it('should handle error when loading users from service failed', () => {
      (userServiceMock.getAllUsers as any).mockReturnValue(throwError(() => ({ status: 500 })));
      component.getAllUsers();
      expect(toastrMock.error).toHaveBeenCalledWith('Ocorreu um erro ao carregar os usuários');
    });
